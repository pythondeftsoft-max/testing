-- Fix all analytics functions to avoid EXTRACT issues
-- Use simpler date arithmetic instead
    ...
    SELECT 
      ...
      CASE 
        WHEN p.status = 'available' THEN 
          (CURRENT_DATE - p.created_at::DATE)::NUMERIC
        ELSE NULL 
...
BEGIN
  ...
  placement_times AS (
    SELECT 
      (placement_date::DATE - requested_at::DATE)::NUMERIC as days_to_placement
    FROM request_stats 
...
BEGIN
  ...
  maintenance_resolution AS (
    SELECT 
      AVG(completed_date::DATE - created_at::DATE)::NUMERIC as avg_resolution_days
