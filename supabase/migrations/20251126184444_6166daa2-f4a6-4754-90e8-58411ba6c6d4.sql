-- Fix landlord_reject_primary_applicant to reset pipeline_stage when rejecting primary applicant
...
BEGIN
  ...
  IF p_unit_id IS NOT NULL THEN
    ...
    -- Update unit: clear primary applicant, set status back to active, and reset pipeline stage
  ...
  -- If only property_id is provided, find the unit with primary applicant
  ELSIF p_property_id IS NOT NULL THEN
    -- Find the unit that has a primary applicant for this property
    SELECT pu.id, pu.property_id, pu.primary_applicant_id, p.owner_id, pu.status
    INTO v_unit_id, v_property_id, v_tenant_id, v_landlord_id, v_old_status
    FROM property_units pu
    JOIN properties p ON pu.property_id = p.id
    ...
    IF v_unit_id IS NULL THEN
      RAISE EXCEPTION 'No unit with primary applicant found for this property or you do not own this property';
    END IF;

    -- Update the unit: clear primary applicant, set status back to active, and reset pipeline stage
    UPDATE property_units
    SET 
      primary_applicant_id = NULL,
      status = 'active',
