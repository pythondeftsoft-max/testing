...
-- Add tax form status tracking columns to existing table
...
-- Add W-9 expiration tracking columns to existing table
...
BEGIN
  ...
  -- Get the property_id for the payer
  SELECT id INTO payer_property_id FROM public.properties WHERE id = NEW.property_id;
  
  -- If payment meets threshold, ensure tax tracking exists
  IF NEW.amount >= threshold_amount AND payer_property_id IS NOT NULL THEN
...
-- Update 1099 forms RLS using correct column structure
...
TO authenticated
USING (
  -- User is the payer OR has portfolio access
