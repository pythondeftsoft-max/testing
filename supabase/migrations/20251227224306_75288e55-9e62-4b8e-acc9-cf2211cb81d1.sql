-- Drop and recreate landlord_reject_primary_applicant to handle landlord_review denials
...
BEGIN
  ...
  -- First try to get the primary applicant
  SELECT primary_applicant_id INTO v_user_id
  FROM property_units
  WHERE id = v_unit_id;
  
  -- If no primary applicant, look for a tenant in landlord_review status
  ...
  -- Clear primary applicant from unit (if it was set)
  ...
  WHERE unit_id = v_unit_id
    ...
    AND status IN ('push_sent', 'interested', 'landlord_review');
...
-- Fix the currently stuck push for test2 tenant
UPDATE property_pushes
SET status = 'denied', updated_at = NOW()
WHERE id = '7f5e945d-45e4-4d59-b6d2-a9d17c2a43c5';