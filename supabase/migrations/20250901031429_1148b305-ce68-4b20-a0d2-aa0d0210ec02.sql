...
-- Fix type mismatches in get_all_portfolios_with_roles by casting to text
...
BEGIN
    ...
    SELECT
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'user_id', pr.user_id,
          'user_name', COALESCE(pp.first_name || ' ' || pp.last_name, pp.company_name, au.email)::text,
          'user_email', au.email::text,
  ...
  SELECT 
    ...
    p.client_name::text AS portfolio_name,
    p.manager_id AS owner_id,
    COALESCE(owner.first_name || ' ' || owner.last_name, owner.company_name, owner_user.email::text)::text AS owner_name,
    owner_user.email::text AS owner_email,
    p.created_at,
    COALESCE(pc.cnt, 0)::int AS property_count,
    COALESCE(rd.active_role_count, 0)::int AS role_count,
