-- First, let's get the actual landlord user ID
DO $$
DECLARE
    landlord_user_id UUID;
BEGIN
    -- Get the landlord user ID
    SELECT id INTO landlord_user_id FROM auth.users WHERE email = 'landlord@openkey.com';
    
    -- If landlord doesn't exist, create a placeholder
    IF landlord_user_id IS NULL THEN
        -- Create a placeholder landlord profile
        ...
        landlord_user_id := 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa';
    ELSE
    ...
    DELETE FROM properties WHERE owner_id = landlord_user_id;
    DELETE FROM profiles WHERE user_type = 'tenant';
