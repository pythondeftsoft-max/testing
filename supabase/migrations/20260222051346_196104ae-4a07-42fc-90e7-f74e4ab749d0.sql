...
-- Add desired location columns to tenant_profiles
ALTER TABLE public.tenant_profiles
  ADD COLUMN IF NOT EXISTS desired_city text,
  ADD COLUMN IF NOT EXISTS desired_state text,
  ADD COLUMN IF NOT EXISTS desired_zip_code text;

-- Update handle_new_user trigger to extract desired location from metadata
...
BEGIN
  ...
  -- Upsert profile
  INSERT INTO public.profiles (id, first_name, last_name, role, phone, country_code)
