-- Fix rejected_by assignment in landlord_send_lease function to use landlord UUID instead of string
...
DECLARE
  ...
  v_unit_id UUID;
BEGIN
  -- Get application details including property address
  SELECT 
    pa.property_id,
    pa.tenant_id,
    p.owner_id,
  ...
  -- Reject all other applications to this unit/property
  UPDATE property_applications
  SET 
    status = 'rejected',
  ...
  -- Withdraw all other applications by this tenant
  UPDATE property_applications
  SET 
    status = 'withdrawn',
    withdrawal_reason = 'Accepted lease for another property',
    withdrawn_at = NOW(),
    updated_at = NOW()
