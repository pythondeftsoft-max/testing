-- Fix landlord_set_primary_applicant to check marketplace_applications first
-- and properly reject old primary applicants
...
BEGIN
  ...
  -- FIXED: Check marketplace_applications FIRST for existing primary
  ...
  -- If not found, also check unit_applications as fallback
  ...
  -- For single-family properties, also pause the property listing
...
-- Fix corrupted data: Reject Logan Rodriguez who should have been rejected
UPDATE marketplace_applications 
SET 
  is_primary_applicant = false, 
  status = 'rejected'
WHERE property_id = '401a568f-75be-4670-8007-96d688fe358d' 
  AND user_id = '03e26106-4179-4b55-bd38-66c5414e8ba2';