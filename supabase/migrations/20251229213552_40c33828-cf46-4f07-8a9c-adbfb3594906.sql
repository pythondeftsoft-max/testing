...
BEGIN
  ...
  SELECT ma.applicant_id, ma.property_id, ma.is_primary_applicant
  ...
  -- Try property_pushes (admin push applications) - NOW WITH PRIMARY APPLICANT SUPPORT
  ...
  IF v_property_id IS NOT NULL THEN
    ...
    IF v_landlord_owns THEN
      -- If primary applicant push, unlimited messaging
      IF v_is_primary THEN
        RETURN QUERY SELECT true, 999999, NULL::text, true;
        RETURN;
      END IF;
      
      -- Count landlord messages using property_push_id column
      SELECT COUNT(*)::integer INTO v_message_count
      FROM messages m
  ...
  -- No matching application found or landlord doesn't own property
