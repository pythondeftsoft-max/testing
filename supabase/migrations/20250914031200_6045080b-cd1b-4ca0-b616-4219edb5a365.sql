-- Fix accounts receivable system with proper balance calculations (corrected)
...
BEGIN
  ...
  INTO v_monthly_rent, v_lease_start_date, v_status
  FROM properties 
  ...
  v_total_due := v_monthly_rent * GREATEST(v_months_due, 0);
  
  -- Add any additional charges from accounts_receivable_details
  SELECT COALESCE(v_total_due + SUM(amount), v_total_due) INTO v_total_due
