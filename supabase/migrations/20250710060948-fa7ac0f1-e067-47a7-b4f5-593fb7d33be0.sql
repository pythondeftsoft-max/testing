...
-- Migrate existing properties to create default units (fixed approach)
...
SELECT 
  ...
  CASE 
    ...
    ELSE 'Unit ' || s.unit_num::text
  END,
  CASE 
    WHEN p.unit_count = 1 THEN p.address
    ELSE p.address || ' - Unit ' || s.unit_num::text
  END,
  p.monthly_rent,
  p.bedrooms,
  p.bathrooms,
  CASE 
    WHEN p.status = 'occupied' AND s.unit_num = 1 THEN 'occupied'
    ELSE 'available'
  END,
...
CROSS JOIN LATERAL generate_series(1, p.unit_count) AS s(unit_num)
