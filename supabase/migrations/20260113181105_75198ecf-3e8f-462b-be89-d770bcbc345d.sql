-- Update admin_get_properties_overview to filter out deleted properties and return on_market
...
BEGIN
  ...
  GROUP BY p.id, p.address, p.monthly_rent, p.status, p.owner_id, p.portfolio_id, 
           prof.first_name, prof.last_name, p.created_at, p.deleted_at, p.lease_end_date, p.on_market
