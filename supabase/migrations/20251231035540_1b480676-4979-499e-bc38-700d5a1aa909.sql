-- First, drop ALL existing versions of landlord_send_lease by OID
DO $$
DECLARE
  func_oid oid;
BEGIN
  FOR func_oid IN 
    ...
    WHERE n.nspname = 'public' AND p.proname = 'landlord_send_lease'
  LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS %s CASCADE', func_oid::regprocedure);
  END LOOP;
END $$;

-- Create the ONE AND ONLY version of landlord_send_lease
...
BEGIN
  ...
  v_placement_fee := v_monthly_rent / 2;
