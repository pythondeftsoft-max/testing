-- Drop and recreate landlord_send_lease function with correct column name (user_id instead of applicant_id)
...
DECLARE
  ...
  v_fee_percentage numeric := 0.50; -- 50% placement fee
  v_message_id uuid;
BEGIN
  ...
  IF v_source_type IS NULL THEN
    SELECT 'property', pa.property_id, pa.unit_id, pa.applicant_id, p.owner_id
    INTO v_source_type, v_property_id, v_unit_id, v_tenant_id, v_landlord_id
  ...
  -- Verify the current user is the landlord
  IF auth.uid() != v_landlord_id THEN
    RAISE EXCEPTION 'Unauthorized: Only the property owner can send a lease';
  END IF;

  -- Use provided unit_id if available, otherwise use the one from application
  IF p_unit_id IS NOT NULL THEN
    v_unit_id := p_unit_id;
  END IF;

  -- Get monthly rent from unit or property
  ...
  -- Send automated message to tenant about lease
