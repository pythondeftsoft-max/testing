...
BEGIN
  ...
  -- Safe type casts: text -> numeric/integer with regex validation
  IF v_user_type = 'tenant' THEN
    INSERT INTO public.tenant_profiles (
      ...
      v_employment_status,
      -- Safe numeric cast for monthly_income
      CASE WHEN v_monthly_income IS NOT NULL AND v_monthly_income ~ '^\d+(\.\d+)?$'
           THEN v_monthly_income::numeric ELSE NULL END,
      v_has_eviction,
      v_eviction_details,
      v_has_pets,
      v_pet_type,
