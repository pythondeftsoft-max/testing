-- Drop the incorrectly created user_points table if it was attempted
-- and create portfolio_user_points table instead

-- Create portfolio_user_points table to track distributed portfolio points to individual users
CREATE TABLE public.portfolio_user_points (
...
-- Add RLS policies for portfolio_user_points
ALTER TABLE public.portfolio_user_points ENABLE ROW LEVEL SECURITY;

-- Users can view their own distributed points
CREATE POLICY "Users can view their own portfolio user points" ON public.portfolio_user_points
  FOR SELECT USING (user_id = auth.uid());

-- Portfolio admin_partners can manage distributed points
CREATE POLICY "Portfolio admin_partners can manage portfolio user points" ON public.portfolio_user_points
  FOR ALL USING (
...
CREATE POLICY "Portfolio members can view portfolio user points" ON public.portfolio_user_points
  FOR SELECT USING (
...
CREATE POLICY "System can insert portfolio user points" ON public.portfolio_user_points
  FOR INSERT WITH CHECK (true);

-- Update the distribute function to use the correct table name
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.portfolio_user_points (
      user_id,
      portfolio_id,
      portfolio_points_id,
      points_awarded,
...
-- Update the summary function to use the correct table name
...
CREATE INDEX idx_portfolio_user_points_user_id ON public.portfolio_user_points(user_id);
CREATE INDEX idx_portfolio_user_points_portfolio_id ON public.portfolio_user_points(portfolio_id);
CREATE INDEX idx_portfolio_user_points_portfolio_points_id ON public.portfolio_user_points(portfolio_points_id);
CREATE INDEX idx_portfolio_user_points_created_at ON public.portfolio_user_points(created_at);
CREATE INDEX idx_portfolio_user_points_source_event ON public.portfolio_user_points(source_event_type);

-- Create composite index for user portfolio queries
CREATE INDEX idx_portfolio_user_points_user_portfolio ON public.portfolio_user_points(user_id, portfolio_id);