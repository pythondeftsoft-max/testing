-- Add sms_consent column to tenant_profiles
ALTER TABLE public.tenant_profiles ADD COLUMN IF NOT EXISTS sms_consent boolean DEFAULT false;

-- Update handle_new_user trigger to extract sms_consent from metadata
-- First check current trigger function and add sms_consent extraction
...
BEGIN
  ...
  v_user_type := meta->>'user_type';
  v_first_name := meta->>'first_name';
  v_last_name := meta->>'last_name';
  v_phone := meta->>'phone';
  v_company_name := meta->>'company_name';
  v_referral_code := meta->>'referral_code';
  v_tenant_intent := meta->>'tenant_intent';
  v_sms_consent := COALESCE((meta->>'sms_consent')::boolean, false);
  ...
  -- Handle bedrooms_approved (JSON array to text array)
  ...
  INSERT INTO public.profiles (id, first_name, last_name, company_name, phone, user_type, referral_code)
  VALUES (
    NEW.id,
    v_first_name,
    v_last_name,
  ...
  -- Upsert into tenant_profiles with ALL fields (atomic save)
