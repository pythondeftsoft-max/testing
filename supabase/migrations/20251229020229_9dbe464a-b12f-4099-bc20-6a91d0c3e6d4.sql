-- Fix parameter order to match front-end: (p_application_id, p_tenant_id)
DROP FUNCTION IF EXISTS public.check_messaging_quota(uuid, uuid);

CREATE FUNCTION public.check_messaging_quota(p_application_id uuid, p_tenant_id uuid)
...
BEGIN
  ...
  -- Count messages sent by tenant for this property
