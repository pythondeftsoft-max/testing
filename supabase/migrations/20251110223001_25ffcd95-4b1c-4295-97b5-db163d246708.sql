-- Fix portfolio metrics function with correct role types and casting
...
BEGIN
  ...
  WITH property_data AS (
    ...
    WHERE (p_portfolio_id IS NULL OR p.portfolio_id = p_portfolio_id)
      ...
      AND (
        p.owner_id = auth.uid() 
        OR (p.portfolio_id IS NOT NULL AND has_portfolio_role(p.portfolio_id, auth.uid(), ARRAY['admin_partner', 'editor', 'viewer']::portfolio_role_type[]))
      )
