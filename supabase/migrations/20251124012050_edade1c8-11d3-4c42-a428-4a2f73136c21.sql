-- Fix column name from application_status to status in landlord_set_primary_applicant
...
BEGIN
  ...
  -- Create or update unit_applications record using profiles.id (p_tenant_id)
  ...
  VALUES (
    p_tenant_id,  -- Use profiles.id, not tenant_profiles.id
    p_unit_id,
    'approved',
    true,
  ...
  -- Set the new primary applicant and pause the listing (no status change)
  UPDATE property_units
  SET 
  ...
  -- Update the marketplace application status using user_id (FIXED: status not application_status)
