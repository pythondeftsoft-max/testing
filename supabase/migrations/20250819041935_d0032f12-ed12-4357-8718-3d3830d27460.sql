...
-- Create unique index to prevent overlapping active grants (simplified without COALESCE)
CREATE UNIQUE INDEX idx_access_grants_active_unique_no_portfolio 
ON public.access_grants (user_id, scope, object_name, action) 
WHERE expires_at > now() AND portfolio_id IS NULL;

CREATE UNIQUE INDEX idx_access_grants_active_unique_with_portfolio 
ON public.access_grants (user_id, scope, portfolio_id, object_name, action) 
WHERE expires_at > now() AND portfolio_id IS NOT NULL;
