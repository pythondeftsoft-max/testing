-- Fix get_points_recent_activity_admin to query points_history instead of points_transactions
...
-- Fix get_points_system_overview_admin to query points_history
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    COALESCE(SUM(CASE WHEN points_change > 0 THEN points_change ELSE 0 END), 0) as total_points_distributed,
    COUNT(DISTINCT user_id) as active_users,
    CASE 
      WHEN last_month_points > 0 THEN 
        ROUND(((current_month_points::numeric - last_month_points::numeric) / last_month_points::numeric * 100), 2)
      ELSE NULL
    END as monthly_growth,
    ROUND(COALESCE(SUM(CASE WHEN points_change > 0 THEN points_change ELSE 0 END), 0) / points_to_dollar_ratio, 2) as total_value
  FROM points_history;
END;
$$;

-- Fix get_points_monthly_trends_admin to query points_history
...
-- Fix get_points_leaderboard_admin to query points_history
...
BEGIN
  ...
  GROUP BY ph.user_id, p.full_name, au.email
