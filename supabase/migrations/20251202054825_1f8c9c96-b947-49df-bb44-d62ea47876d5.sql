-- Fix notify_new_message function to use owner_id from properties table
-- The properties table uses 'owner_id' not 'landlord_id'
...
BEGIN
  ...
  SELECT email, COALESCE(full_name, first_name || ' ' || last_name, email)
