...
SELECT 
  ...
  po.id,
  true,
  CASE WHEN po.name IN ('properties', 'maintenance_requests') THEN true ELSE false END as can_edit,
  false,
  CASE WHEN po.name = 'maintenance_requests' THEN true ELSE false END as can_create
FROM public.permission_objects po
WHERE po.scope = 'portfolio'
