-- Fix tenant_sign_marketplace_lease to tie tenant to unit and update pipeline stages
...
BEGIN
  ...
  -- Tie tenant to unit and update property pipeline stage
  UPDATE property_units
  SET 
    current_tenant_id = v_application.user_id,
    pipeline_stage = 'filled_awaiting_payment',
    status = 'off_market',
    updated_at = NOW()
  WHERE id = v_application.unit_id;

  -- Update tenant profile pipeline stage
  UPDATE profiles
  SET 
