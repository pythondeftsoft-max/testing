-- Fix the enhanced tenant profile function to only reference existing columns
...
RETURNS TABLE(
  ...
  -- Tenant profile details (only existing columns)
  date_of_birth date,
  ssn text,
...
BEGIN
  ...
  SELECT 
    ...
    -- Tenant profile details (using actual column names)
    tp.date_of_birth,
    tp.ssn,
    tp.address as current_address,
    tp.city as current_city,
    tp.state as current_state,
