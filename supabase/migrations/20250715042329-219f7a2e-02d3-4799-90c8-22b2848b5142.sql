-- Update get_landlord_portfolio_overview function to properly calculate units
...
BEGIN
  ...
  WITH property_stats AS (
    SELECT 
      ...
      COALESCE(SUM(COALESCE(unit_count, 1)), 0) as total_units_count,
      -- Calculate vacant units by checking property status and unit availability
      COALESCE(SUM(
        CASE 
          WHEN status IN ('available', 'vacant') THEN COALESCE(unit_count, 1)
          ELSE 0 
        END
