...
-- Update the toggle_property_market_listing function to clear listing data for vacant properties when taken off market
...
BEGIN
  ...
  SET 
    ...
    pet_policy = CASE 
      WHEN p_on_market = false AND NOT has_approved_tenant THEN NULL
      ELSE COALESCE(p_listing_data->>'pet_policy', pet_policy)
    END,
    amenities = CASE 
      WHEN p_on_market = false AND NOT has_approved_tenant THEN NULL
