-- Fix admin_set_property_market_status function - correct column references
-- The properties table does not have current_tenant_id column
-- The property_units table uses tenant_id, not current_tenant_id
...
BEGIN
  ...
  -- Check if any units have tenants (property-level occupancy derived from units)
  SELECT EXISTS (
    SELECT 1 FROM property_units 
    WHERE property_id = p_property_id AND tenant_id IS NOT NULL
  ) INTO v_has_tenant;
  ...
  -- Update all units for this property (using correct column: tenant_id)
  UPDATE property_units
  SET 
    on_market = p_on_market,
    status = CASE 
      WHEN p_on_market THEN 'available'
      WHEN tenant_id IS NOT NULL THEN 'occupied'
      ELSE 'vacant'
    END,
    updated_at = NOW()
