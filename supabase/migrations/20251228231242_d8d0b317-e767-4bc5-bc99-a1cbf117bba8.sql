-- Drop the existing function first (logic change)
DROP FUNCTION IF EXISTS public.check_landlord_messaging_quota(uuid, uuid);

-- Recreate with 5-message limit for non-primary, unlimited for primary
...
BEGIN
  ...
  -- If found, verify landlord owns the property (using owner_id, not landlord_id)
