-- Drop BOTH versions of the duplicate functions
DROP FUNCTION IF EXISTS get_admin_all_appointments();
DROP FUNCTION IF EXISTS get_admin_all_appointments(uuid);
DROP FUNCTION IF EXISTS get_landlord_appointments();
DROP FUNCTION IF EXISTS get_landlord_appointments(uuid);

-- Recreate get_landlord_appointments with all needed columns
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(prof.full_name, prof.first_name || ' ' || prof.last_name) AS tenant_name
...
-- Recreate get_admin_all_appointments with all needed columns
...
BEGIN
  ...
  SELECT 
    ...
    mr.title AS request_title,
    COALESCE(prof.full_name, prof.first_name || ' ' || prof.last_name) AS tenant_name,
    COALESCE(landlord.full_name, landlord.first_name || ' ' || landlord.last_name) AS landlord_name
  FROM maintenance_appointments ma
  LEFT JOIN properties p ON ma.property_id = p.id
  LEFT JOIN property_units pu ON ma.unit_id = pu.id
  LEFT JOIN maintenance_vendors mv ON ma.vendor_id = mv.id
