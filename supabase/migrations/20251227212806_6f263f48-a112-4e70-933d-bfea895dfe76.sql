-- Drop existing function and recreate with on_market fix
DROP FUNCTION IF EXISTS public.landlord_reject_primary_applicant(UUID, UUID, TEXT);

-- Recreate with fix: use on_market = true instead of listing_status = 'active'
...
BEGIN
  ...
  -- Reactivate the property listing
  UPDATE properties
  SET 
    on_market = true,
    updated_at = NOW()
...
-- Fix 1360 Scotch Mountain Road - set on_market = true
UPDATE properties
SET on_market = true, updated_at = NOW()
WHERE address ILIKE '%1360 Scotch Mountain%';

UPDATE property_units
SET on_market = true, pipeline_stage = 'available', updated_at = NOW()
WHERE property_id IN (
  SELECT id FROM properties WHERE address ILIKE '%1360 Scotch Mountain%'
);