...
-- RLS Policies
CREATE POLICY "Admins can view RBAC logs" 
ON public.rbac_audit_logs 
FOR SELECT 
USING (get_highest_account_role(auth.uid()) IN ('owner', 'admin_partner', 'support_assistant'));

CREATE POLICY "No direct inserts for clients" 
ON public.rbac_audit_logs 
FOR INSERT 
WITH CHECK (false);
...
CREATE INDEX idx_rbac_audit_logs_allowed_created_at ON public.rbac_audit_logs (allowed, created_at DESC);
...
CREATE INDEX idx_rbac_audit_logs_denied ON public.rbac_audit_logs (created_at DESC) WHERE allowed = false;
...
-- Function to purge old RBAC logs
...
DO $$
BEGIN
  -- Try to schedule the purge function
  PERFORM cron.schedule(
    'purge-old-rbac-logs',
    '0 2 * * *', -- Daily at 2 AM
    'SELECT public.purge_old_rbac_logs(90);'
  );
EXCEPTION
