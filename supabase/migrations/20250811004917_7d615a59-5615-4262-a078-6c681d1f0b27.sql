...
create policy if not exists "ptp_read_by_portfolio_roles" on public.portfolio_tax_profiles
for select using (
  has_portfolio_role(portfolio_id, auth.uid(), ARRAY['admin_partner','editor','viewer'])
);

create policy if not exists "ptp_insert_by_portfolio_admins" on public.portfolio_tax_profiles
for insert with check (
  has_portfolio_role(portfolio_id, auth.uid(), ARRAY['admin_partner','owner'])
);

create policy if not exists "ptp_update_by_portfolio_admins" on public.portfolio_tax_profiles
...
create trigger if not exists trg_ptp_updated
...
create policy if not exists "tp_users_view_own" on public.tax_profiles
for select using (user_id = auth.uid());

create policy if not exists "tp_portfolio_view" on public.tax_profiles
for select using (
  portfolio_id is not null and has_portfolio_role(portfolio_id, auth.uid(), ARRAY['admin_partner','editor','viewer'])
);

create policy if not exists "tp_user_manage_own" on public.tax_profiles
for all using (user_id = auth.uid()) with check (user_id = auth.uid());

create policy if not exists "tp_portfolio_admin_manage" on public.tax_profiles
for all using (
...
create trigger if not exists trg_tp_updated
...
create policy if not exists "tt_portfolio_view" on public.tax_transactions
for select using (
  portfolio_id is not null and has_portfolio_role(portfolio_id, auth.uid(), ARRAY['admin_partner','editor','viewer'])
);

create policy if not exists "tt_portfolio_manage" on public.tax_transactions
for all using (
...
create trigger if not exists trg_tt_updated
...
create policy if not exists "tf_portfolio_view" on public.tax_forms_1099
for select using (
  portfolio_id is not null and has_portfolio_role(portfolio_id, auth.uid(), ARRAY['admin_partner','editor','viewer'])
);

create policy if not exists "tf_portfolio_manage" on public.tax_forms_1099
for all using (
  portfolio_id is not null and has_portfolio_role(portfolio_id, auth.uid(), ARRAY['admin_partner','owner'])
) with check (
  portfolio_id is not null and has_portfolio_role(portfolio_id, auth.uid(), ARRAY['admin_partner','owner'])
...
create trigger if not exists trg_tf_updated
...
create policy if not exists "tax_thresholds_read" on public.tax_thresholds for select using (true);
...
create policy if not exists "iris_payloads_view" on public.tax_iris_payloads
for select using (has_portfolio_role(portfolio_id, auth.uid(), ARRAY['admin_partner','editor','viewer']));
create policy if not exists "iris_payloads_manage" on public.tax_iris_payloads
...
create policy if not exists "tax_forms_read" on storage.objects
for select using (
  bucket_id = 'tax-forms'
  and (
...
create policy if not exists "tax_forms_insert" on storage.objects
for insert with check (
  bucket_id = 'tax-forms'
...
create policy if not exists "tax_forms_update" on storage.objects
for update using (
  bucket_id = 'tax-forms'
  and has_portfolio_role((storage.foldername(name))[1]::uuid, auth.uid(), ARRAY['admin_partner','owner'])
);

create policy if not exists "tax_forms_delete" on storage.objects
for delete using (
  bucket_id = 'tax-forms'
  and has_portfolio_role((storage.foldername(name))[1]::uuid, auth.uid(), ARRAY['admin_partner','owner'])
);
