-- FULL SETUP: create tables first, then RLS and triggers

-- 1) portfolio_tax_profiles
...
-- portfolio_tax_profiles policies
...
create trigger trg_ptp_updated before update on public.portfolio_tax_profiles for each row execute function public.update_updated_at_column();

drop trigger if exists trg_tp_updated on public.tax_profiles;
create trigger trg_tp_updated before update on public.tax_profiles for each row execute function public.update_updated_at_column();

drop trigger if exists trg_tt_updated on public.tax_transactions;
create trigger trg_tt_updated before update on public.tax_transactions for each row execute function public.update_updated_at_column();

drop trigger if exists trg_tf_updated on public.tax_forms_1099;
create trigger trg_tf_updated before update on public.tax_forms_1099 for each row execute function public.update_updated_at_column();

-- 9) RPC: compute_1099_candidates
...
-- 10) Storage bucket for tax forms (private)
insert into storage.buckets (id, name, public)
values ('tax-forms','tax-forms', false)
on conflict (id) do nothing;

-- Storage RLS policies with role casts
