-- Change lease_document_id from UUID to TEXT to store storage file paths
ALTER TABLE marketplace_applications 
ALTER COLUMN lease_document_id TYPE TEXT USING lease_document_id::TEXT;

-- Recreate landlord_send_lease function with TEXT parameter for lease_document_id
...
CREATE OR REPLACE FUNCTION public.landlord_send_lease(
  ...
  p_lease_method TEXT,
  p_lease_document_id TEXT DEFAULT NULL,  -- Changed to TEXT to accept storage paths
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(pu.unit_number, pu.unit_name, 'Unit')
  INTO 
    v_property_name,
    v_property_address,
    v_unit_name
  ...
  -- Update application with correct status based on method
  UPDATE marketplace_applications
  SET 
    status = CASE 
      WHEN p_lease_method = 'openkey' THEN 'lease_sent'::application_status
      ELSE 'lease_signed'::application_status
    END,
    lease_method = p_lease_method,
    lease_document_id = p_lease_document_id,
    lease_sent_at = NOW(),
    -- Set tenant_signed_at for uploaded (already signed) leases
    tenant_signed_at = CASE 
      WHEN p_lease_method = 'uploaded' THEN NOW() 
      ELSE tenant_signed_at 
    END,
    updated_at = NOW()
  WHERE id = p_application_id;

  -- Only send message to tenant for 'openkey' method
