...
VALUES 
  (gen_random_uuid(), 'ccb8536c-80d1-4834-9614-169b9a7caede', 'Everything', 'everything@example.com', '555-0001', now(), now()),
  (gen_random_uuid(), 'ccb8536c-80d1-4834-9614-169b9a7caede', 'Test 1', 'test1@example.com', '555-0002', now(), now());
...
VALUES (
  ...
  (SELECT id FROM public.portfolios WHERE client_name = 'Everything' AND manager_id = 'ccb8536c-80d1-4834-9614-169b9a7caede'),
...
SELECT 
  gen_random_uuid(),
  hu.unit_id,
  'ccb8536c-80d1-4834-9614-169b9a7caede', -- Demo landlord as tenant for testing
  'landlord',
  'Demo Landlord',
  true,
  true,
...
-- Create sample HAP payments for December 2024
WITH hap_units AS (
  SELECT pu.id as unit_id, pu.pha_portion, hpc.id as payee_config_id
  FROM public.property_units pu
  JOIN public.properties p ON pu.property_id = p.id
  JOIN public.hap_payee_configs hpc ON pu.id = hpc.unit_id
  WHERE p.street_address = '1234 Riverside Drive' 
    AND p.city = 'Chicago'
    AND pu.has_voucher = true
)
...
-- Create sample HAP payments for January 2025
WITH hap_units AS (
  SELECT pu.id as unit_id, pu.pha_portion, hpc.id as payee_config_id
  FROM public.property_units pu
  JOIN public.properties p ON pu.property_id = p.id
...
-- Create sample HAP payments for February 2025 (expected/pending)
WITH hap_units AS (
  SELECT pu.id as unit_id, pu.pha_portion, hpc.id as payee_config_id
  FROM public.property_units pu
  JOIN public.properties p ON pu.property_id = p.id
...
SET portfolio_id = (
  ...
  WHERE client_name = 'Test 1' AND manager_id = 'ccb8536c-80d1-4834-9614-169b9a7caede'
)
