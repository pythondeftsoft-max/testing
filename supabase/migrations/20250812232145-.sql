-- Re-apply full migration with schema-qualified enum and corrected seed CTE
...
role_matrix AS (
  -- role, object, can_view, can_edit, can_delete, can_create
  SELECT 'owner'::public.account_role_type AS role_name, o.name AS obj, true, true, true, true FROM objs o
  UNION ALL
...
SELECT rm.role_name, o.id,
       rm.true AS can_view,
       rm.column2 AS can_edit,
       rm.column3 AS can_delete,
       rm.column4 AS can_create
FROM role_matrix rm
