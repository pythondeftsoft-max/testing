-- Add sample tenant balances for testing
...
-- Add sample recurring charges for testing
...
AND NOT EXISTS (
  ...
  WHERE rc.property_id = p.id AND rc.charge_type = 'pet_fee'
);
...
AND NOT EXISTS (
  SELECT 1 FROM recurring_charges rc 
  WHERE rc.property_id = p.id AND rc.charge_type = 'utility'
);
