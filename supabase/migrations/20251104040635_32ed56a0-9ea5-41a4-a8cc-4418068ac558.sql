-- Drop the broken function that queries non-existent tables
DROP FUNCTION IF EXISTS get_unified_activity_feed(timestamptz, timestamptz, text, text, text, text, text);

-- Create the corrected function that queries the ACTUAL tables
...
BEGIN
  ...
  WITH rbac_logs AS (
    ...
    LEFT JOIN profiles p ON p.user_id = rel.user_id
  ...
  account_logs AS (
    ...
    LEFT JOIN profiles p ON p.user_id = aal.user_id
    WHERE aal.created_at BETWEEN v_start_date AND v_end_date
  ),
  unified AS (
