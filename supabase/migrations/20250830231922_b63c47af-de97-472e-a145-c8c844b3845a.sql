...
BEGIN
  ...
  INSERT INTO admin_audit_logs (
    ...
    jsonb_build_object(
      'reason', p_reason,
      'property_address', property_record.address,
      'tenant_name', CONCAT(tenant_record.first_name, ' ', tenant_record.last_name),
...
BEGIN
  ...
  INSERT INTO admin_audit_logs (
    ...
    jsonb_build_object(
      'reason', p_reason,
      'property_address', property_record.address,
      'old_tenant_name', CASE 
        WHEN old_tenant_record.first_name IS NOT NULL THEN 
          CONCAT(old_tenant_record.first_name, ' ', old_tenant_record.last_name)
        ELSE 'None'
      END,
      'old_tenant_email', COALESCE(old_tenant_record.email, 'None'),
      'new_tenant_name', CONCAT(new_tenant_record.first_name, ' ', new_tenant_record.last_name),
      'new_tenant_email', new_tenant_record.email,
      'metadata', p_metadata
    )
  );
...
-- Create RLS policy for admin audit logs
