-- Fix landlord_send_lease function to use UPSERT for placement fees
...
BEGIN
  ...
  -- UPSERT placement fee - update if exists, insert if not
  ...
  ON CONFLICT ON CONSTRAINT unique_pending_placement_fee
  DO UPDATE SET
    landlord_id = EXCLUDED.landlord_id,
    marketplace_application_id = EXCLUDED.marketplace_application_id,
    application_id = EXCLUDED.application_id,
    first_month_rent = EXCLUDED.first_month_rent,
    fee_amount = EXCLUDED.fee_amount,
    due_date = EXCLUDED.due_date,
    updated_at = now();

  -- Insert message with correct schema
  ...
  -- Return fields matching frontend SendLeaseResponse interface
