-- Update handle_new_user trigger to save ALL tenant profile fields from signup metadata
-- This ensures reliable data saving without relying on RLS timing
...
BEGIN
  ...
  -- If landlord, ensure they have an entry in the portfolios table (unchanged logic)
  IF v_user_type = 'landlord'::public.user_type 
     AND to_regclass('public.portfolios') IS NOT NULL THEN
    INSERT INTO public.portfolios (landlord_id, name, description)
    VALUES (
      NEW.id,
      COALESCE(NEW.raw_user_meta_data ->> 'company_name', 'My Portfolio'),
      'Default portfolio created on signup'
    )
...
-- Also backfill any existing tenant_profiles with NULL boolean fields to prevent modal from showing
...
SET 
  ...
  has_accessibility_needs = COALESCE(has_accessibility_needs, false)
WHERE has_eviction IS NULL 
   OR has_pets IS NULL 
   OR has_felonies IS NULL 
   OR has_accessibility_needs IS NULL;