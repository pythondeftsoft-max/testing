...
CREATE INDEX IF NOT EXISTS idx_asset_price_history_timestamp ON public.asset_price_history(timestamp DESC);
...
AS $$
BEGIN
    INSERT INTO public.asset_price_history (symbol, interval_type, timestamp, open, high, low, close, volume, data_source)
    SELECT 
        p_symbol,
        p_interval,
    ...
    ON CONFLICT (symbol, interval_type, timestamp) 
    DO UPDATE SET 
        open = EXCLUDED.open,
        high = EXCLUDED.high,
        low = EXCLUDED.low,
...
BEGIN
    ...
    WHERE aph.symbol = p_symbol 
        ...
        AND aph.timestamp >= p_min_date_time
