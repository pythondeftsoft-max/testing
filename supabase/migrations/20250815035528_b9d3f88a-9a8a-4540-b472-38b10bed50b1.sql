...
-- Update trigger to calculate total_cost on commercial properties
CREATE OR REPLACE FUNCTION calculate_commercial_total_cost()
RETURNS TRIGGER AS $$
BEGIN
  -- Only calculate for commercial properties with square footage
  IF NEW.property_type = 'commercial' AND NEW.leasable_square_footage IS NOT NULL THEN
    NEW.monthly_rent := COALESCE(NEW.base_rent_psf, 0) * COALESCE(NEW.leasable_square_footage, 0) / 12;
  END IF;
...
CREATE TRIGGER trigger_calculate_commercial_total_cost
  BEFORE INSERT OR UPDATE ON public.properties
  FOR EACH ROW
  EXECUTE FUNCTION calculate_commercial_total_cost();
