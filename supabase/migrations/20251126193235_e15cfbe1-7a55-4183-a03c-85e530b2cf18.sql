-- Fix send_tenant_message function with correct column names
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM marketplace_applications WHERE id = application_id) THEN
    ...
    SELECT property_id, landlord_id INTO v_property_id, v_landlord_id 
    FROM property_units WHERE id = v_unit_id;
  ...
  ELSIF EXISTS (SELECT 1 FROM property_applications WHERE id = application_id) THEN
    ...
    SELECT landlord_id INTO v_landlord_id FROM properties WHERE id = v_property_id;
  ELSE
