...
-- Fixed migration based on actual table structure

INSERT INTO property_units (id, property_id, unit_number, bedrooms, bathrooms, square_feet, monthly_rent, status, tenant_id, lease_start_date, lease_end_date)
SELECT 
  gen_random_uuid(),
  p.id,
  '1',
...
AND NOT EXISTS (
  SELECT 1 FROM property_units pu WHERE pu.property_id = p.id AND pu.unit_number = '1'
)
LIMIT 10;
...
SELECT 
  ...
  pu.property_id,
  pu.tenant_id,
  COALESCE(pu.monthly_rent, 1400) * 0.8, -- 80% payment
...
SELECT 
  ...
  pu.tenant_id,
  CASE 
    WHEN random() < 0.3 THEN (COALESCE(pu.monthly_rent, 1400) * 0.5)
    ELSE 0
  END,
  CASE 
    WHEN random() < 0.3 THEN (COALESCE(pu.monthly_rent, 1400) * 0.3)
    ELSE 0
  END,
