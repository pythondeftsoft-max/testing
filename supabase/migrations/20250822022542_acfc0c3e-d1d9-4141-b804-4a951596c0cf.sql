...
CREATE POLICY "Asset owners can view transactions for their assets" 
ON public.asset_payment_transactions 
FOR SELECT 
USING (
  EXISTS (
    ...
    AND (p.owner_id = auth.uid() OR has_portfolio_role(p.id, auth.uid(), ARRAY['admin_partner'::portfolio_role_type, 'editor'::portfolio_role_type]))
  )
);
...
-- Enhance asset_invitations table with lifecycle improvements
ALTER TABLE public.asset_invitations 
