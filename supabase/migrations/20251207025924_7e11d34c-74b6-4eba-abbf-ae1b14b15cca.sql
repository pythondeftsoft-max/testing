-- Update toggle_unit_market_listing to also update status field when toggling market listing
...
BEGIN
  ...
  -- Determine new status based on market toggle (only for non-occupied units)
  v_new_status := v_unit.status;
  
  IF p_on_market = TRUE AND v_unit.status = 'vacant' THEN
    -- Going ON market: vacant -> available
    v_new_status := 'available';
  ELSIF p_on_market = FALSE AND v_unit.status = 'available' THEN
    -- Going OFF market: available -> vacant
    v_new_status := 'vacant';
  END IF;

  -- Update unit on_market status and status field
  ...
  IF p_listing_data IS NOT NULL THEN
    ...
    SET
      ...
      availability_date = COALESCE((p_listing_data->>'availability_date')::DATE, availability_date),
      updated_at = NOW()
    WHERE id = p_unit_id;
  END IF;
