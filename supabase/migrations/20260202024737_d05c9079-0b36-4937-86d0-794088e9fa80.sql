-- Fix trigger_property_match_version to remove references to non-existent columns
-- Both pets_allowed AND available_date do not exist on property_units table
CREATE OR REPLACE FUNCTION public.trigger_property_match_version()
RETURNS TRIGGER AS $$
BEGIN
  -- Check if any match-relevant fields changed
  -- NOTE: pets_allowed and available_date columns do not exist on property_units
  -- Only checking fields that actually exist on this table
