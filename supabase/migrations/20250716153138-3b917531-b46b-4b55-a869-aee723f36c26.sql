-- Update the landlord portfolio overview function to count units instead of properties
...
BEGIN
  ...
  WITH unit_stats AS (
    ...
    WHERE p.owner_id = landlord_id 
      ...
      AND (portfolio_id IS NULL OR p.portfolio_id = portfolio_id)
  ),
  payment_stats AS (
    SELECT 
  ...
  SELECT 
    us.total_unit_count::INTEGER as total_units,
    us.vacant_unit_count::INTEGER as vacant_units,
    CASE 
      WHEN us.total_unit_count > 0 THEN (us.available_unit_count::NUMERIC / us.total_unit_count::NUMERIC * 100)
      ELSE 0 
    END as availability_rate,
    us.available_unit_count::INTEGER as available_units,
    CASE 
      WHEN us.total_unit_count > 0 THEN (us.vacant_unit_count::NUMERIC / us.total_unit_count::NUMERIC * 100)
      ELSE 0 
    END as vacancy_rate,
    COALESCE(us.total_gross_rent, 0)::NUMERIC as gross_rent,
    COALESCE(pys.total_collected, 0)::NUMERIC as collected_rent,
    CASE 
      WHEN us.total_gross_rent > 0 THEN (pys.total_collected / us.total_gross_rent * 100)
      ELSE 0 
    END as collection_rate,
    (COALESCE(us.total_gross_rent, 0) - COALESCE(e.total_expenses, 0))::NUMERIC as net_operating_income,
    COALESCE(tom.avg_days, 0)::NUMERIC as avg_time_on_market
  FROM unit_stats us
