-- Update check_messaging_quota to exclude messages from terminal applications/pushes
...
BEGIN
  ...
  ELSE
    ...
    IF v_property_id IS NULL THEN
      -- Check marketplace_applications
      SELECT ma.property_id, ma.status,
             CASE WHEN ma.status = 'primary_applicant' THEN true ELSE false END
      INTO v_property_id, v_application_status, v_is_primary
    ...
    AND (
      ...
      m.property_application_id IN (
        ...
        WHERE pa.property_id = v_property_id 
          ...
          AND (ma.id IS NULL OR ma.status NOT IN (SELECT unnest(v_terminal_marketplace_statuses)))
      )
      OR
      -- Messages from active marketplace applications
      m.marketplace_application_id IN (
        SELECT ma.id FROM marketplace_applications ma
        WHERE ma.property_id = v_property_id 
          AND ma.user_id = p_tenant_id
          AND ma.status NOT IN (SELECT unnest(v_terminal_marketplace_statuses))
      )
    );
