-- Fix get_portfolio_metrics to only sum rent from OCCUPIED units
...
BEGIN
  ...
  SELECT 
    ...
    -- Only sum rent from OCCUPIED units
    COALESCE(SUM(CASE WHEN ud.is_occupied = 1 THEN ud.monthly_rent ELSE 0 END), 0)::NUMERIC as monthly_profit,
    COALESCE(SUM(CASE WHEN ud.is_occupied = 1 THEN ud.monthly_rent ELSE 0 END), 0)::NUMERIC as gross_monthly_rent
  FROM unit_data ud;
END;
$$;