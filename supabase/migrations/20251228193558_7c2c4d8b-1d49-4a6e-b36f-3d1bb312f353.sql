-- Fix landlord_reject_primary_applicant to also set is_primary_applicant = false
...
BEGIN
  ...
  -- Verify landlord owns this unit (use owner_id, not landlord_id)
  ...
  -- Reject any related marketplace applications AND clear is_primary_applicant
  ...
  WHERE unit_id = v_unit_id
    ...
    AND status IN ('submitted', 'primary_applicant', 'draft');
  
  -- Also clear is_primary_applicant in property_applications
  ...
  -- Reactivate the unit listing AND reset pipeline stage/status
  UPDATE property_units
  SET 
    on_market = true,
    pipeline_stage = CASE 
...
-- Fix existing stuck marketplace application that was rejected but still shows is_primary_applicant = true
UPDATE marketplace_applications
SET is_primary_applicant = false
WHERE unit_id = '540aee3e-19f6-4671-8e7e-10df3960914f'
  AND is_primary_applicant = true;

-- Also fix any other stuck applications across the system
UPDATE marketplace_applications
SET is_primary_applicant = false
WHERE is_primary_applicant = true
  AND status IN ('rejected', 'withdrawn', 'denied');