-- Fix landlord_set_primary_applicant function to use correct column names
-- unit_applications uses tenant_id, marketplace_applications uses user_id
...
BEGIN
  ...
  -- unit_applications uses tenant_id
  ...
  IF v_current_primary_id IS NOT NULL AND v_current_primary_id != p_tenant_id THEN
    ...
    -- marketplace_applications uses user_id
    UPDATE marketplace_applications
    SET is_primary_applicant = false
    WHERE property_id = v_property_id AND user_id = v_current_primary_id;
  END IF;

  -- Set the new primary applicant in unit_applications (uses tenant_id)
  UPDATE unit_applications
  SET 
  ...
  -- Set the new primary applicant in marketplace_applications (uses user_id)
  UPDATE marketplace_applications
