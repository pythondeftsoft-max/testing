-- 1) Add sort_order to permission_objects to fix UI ordering and missing column errors
ALTER TABLE public.permission_objects
  ADD COLUMN IF NOT EXISTS sort_order integer NOT NULL DEFAULT 0;

-- 2) Ensure role_permissions table exists with proper constraints
...
-- 3) Enable RLS and add policies (read for authenticated; manage for owners/admin partners)
...
-- 4) Update functions to include sort ordering and owner override
...
BEGIN
  ...
  IF user_role = 'owner'::account_role_type THEN
    RETURN true;
  END IF;
...
role_matrix AS (
  -- role, object, view, edit, delete, create
  SELECT 'owner'::account_role_type AS role_name, o.name, true, true, true, true FROM objs o
  UNION ALL
  SELECT 'admin_partner'::account_role_type, 'permissions', true, false, false, false
  UNION ALL
  SELECT 'admin_partner'::account_role_type, 'user_management', true, false, false, true
  UNION ALL
  SELECT 'admin_partner'::account_role_type, 'hap_payments', true, false, false, false
  UNION ALL
  SELECT 'admin_partner'::account_role_type, 'rent_payments', true, false, true, false
  UNION ALL
  SELECT 'admin_partner'::account_role_type, 'rent_splits', true, false, false, true
  UNION ALL
  SELECT 'admin_partner'::account_role_type, 'subscriptions', true, false, false, false
  UNION ALL
  SELECT 'support_assistant'::account_role_type, 'permissions', true, false, false, false
  UNION ALL
  SELECT 'support_assistant'::account_role_type, 'user_management', true, false, false, false
  UNION ALL
  SELECT 'support_assistant'::account_role_type, 'hap_payments', true, false, false, false
  UNION ALL
  SELECT 'support_assistant'::account_role_type, 'rent_payments', true, false, false, false
  UNION ALL
  SELECT 'support_assistant'::account_role_type, 'rent_splits', true, false, false, false
  UNION ALL
  SELECT 'support_assistant'::account_role_type, 'subscriptions', true, false, false, false
)
INSERT INTO public.role_permissions (role_name, permission_object_id, can_view, can_edit, can_delete, can_create)
SELECT rm.role_name, o.id,
       rm.true AS can_view,
       rm.edit AS can_edit,
       rm.delete AS can_delete,
       rm.create AS can_create
FROM role_matrix rm
JOIN objs o ON o.name = rm.name
