...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { housing_authority_id, to, subject, body, template_key } = parsed.data;
    ...
    // Ensure prospect status row exists and get its id.
    const { data: existing } = await supabase
      .from("pha_prospect_status")
      .select("id, status")
      .eq("housing_authority_id", housing_authority_id)
      .maybeSingle();

    let prospectId = existing?.id;
    const newStatus = existing?.status === "cold" || !existing ? "contacted" : existing.status;
    const { data: upserted, error: upsertErr } = await supabase
      ...
      .single();
    if (upsertErr) {
      console.error("upsert prospect failed", upsertErr);
    } else {
    ...
    if (prospectId) {
      await supabase.from("pha_prospect_notes").insert({
        ...
        kind: "email",
        note: `Sent: ${subject}`,
        metadata: { to, template_key, resend_id: sendJson?.id, body_preview: body.slice(0, 200) },
      });
    }
